  ...
  try {
    ...
    try {
      ...
      if (nancyProfileContext) {
        console.log(`[reflection-agent] Nancy profile context built (${nancyProfileContext.length} chars, hasData=${nancyProfile.has_real_data}, growth=${nancyProfile.growth_summary ? "yes" : "no"})`);
      }
    } catch (profileErr) {
