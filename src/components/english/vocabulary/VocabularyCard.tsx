...
// Display-only: normalize PDF artifacts (Kangxi radicals, stray IPA spacing). Stored source stays raw.
...
export default function VocabularyCard({word:w,model,onTest}:{word:VocabularyWord;model:ReturnType<typeof useVocabulary>;onTest:(mode:TestMode)=>Promise<void>}) {
  ...
  return <article className="rounded-lg border border-border bg-card p-5 space-y-4 min-w-0">
    ...
    <details className="text-sm"><summary className="cursor-pointer">原文来源（{w.sources.length}）</summary>{w.sources.map((s,i)=><blockquote key={i} className="mt-3 border-l-2 border-border pl-3"><p className="text-xs text-ink-lighter">{s.name} · 原形式 {display(s.original)}</p><p className="whitespace-pre-wrap mt-1">{display(s.context)}</p></blockquote>)}</details>
