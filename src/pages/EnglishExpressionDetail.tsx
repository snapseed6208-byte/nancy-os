...
export default function EnglishExpressionDetail() {
  ...
  const [form, setForm] = useState({
    ...
    context: "",
    common_patterns: "",
    alternative_expressions: [] as AlternativeExpression[],
    category_id: "",
  });
  ...
  useEffect(() => {
    if (existing && !isNew) {
      setForm({
        ...
        alternative_expressions: normalizeAlternativeExpressions(existing.alternative_expressions),
  ...
  const handleSave = async () => {
    ...
    if (isNew) {
      await createExpr.mutateAsync({
        ...form,
        alternative_expressions: normalizeAlternativeExpressions(form.alternative_expressions),
      });
    } else {
      await updateExpr.mutateAsync({
        id,
        ...form,
        alternative_expressions: normalizeAlternativeExpressions(form.alternative_expressions),
      });
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <AlternativeExpressionsField
          value={form.alternative_expressions}
          onChange={(alternative_expressions) => setForm((prev) => ({ ...prev, alternative_expressions }))}
        />
        {!isNew && form.alternative_expressions.length === 0 && (
          <AlternativeExpressionsList
            alternatives={[]}
            legacySynonyms={(existing?.synonyms as string | null) || null}
          />
        )}
