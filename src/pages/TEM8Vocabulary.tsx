  ...
  return <div className="space-y-6 min-w-0 [overflow-wrap:anywhere]">
    ...
    {rejects.length>0&&<details className="rounded-lg border border-border p-3 text-sm"><summary className="cursor-pointer text-ink-light">被拒绝的词条（{rejects.length}）</summary><ul className="mt-2 space-y-1 text-xs text-ink-lighter">{rejects.slice(0,200).map((r,i)=><li key={`${r.span_id}-${r.index}-${i}`}>第 {r.span_id?.split(":")[0]??"?"} 批 · 条目 {r.index} · {r.span_id??"无 span"} · {r.field} · {r.reason}</li>)}</ul></details>}
      ...
      {view === "today" && <section className="rounded-lg bg-sage-light/25 p-5 space-y-3">
        ...
        {model.plan.data ? <p className="text-sm text-ink-light">新词 {model.plan.data.new_ids.length} · 熟词生义 {model.plan.data.familiar_ids.length} · 输出词 {model.plan.data.production_ids.length}{queue.length === 0 && " · 今日学习已完成"}</p> : <div className="flex flex-wrap items-center gap-3"><label className="text-sm">新词名额 <input aria-label="每日新词名额" type="number" min={0} max={100} value={target} onChange={ev => setTarget(Math.max(0, Math.min(100, Number(ev.target.value))))} className="w-20 ml-2 p-2 border border-border rounded-lg" /></label><button disabled={model.startDay.isPending || !words.length} className={button} onClick={() => void run(async () => { if(target >= 1 && target <= 30) saveLearnTarget(target); await model.startDay.mutateAsync(target); })}>生成今日名单</button></div>}
      </section>}
