...
export default function EnglishArticleReader() {
  ...
  return (
    <div className="w-full max-w-full min-w-0 -mt-2 pb-20">
      <header className="sticky top-14 lg:top-0 z-20 box-border bg-warm-cream/95 backdrop-blur border-b border-border -mx-4 px-3 py-2">
        <div className="flex w-full max-w-full min-w-0 items-center gap-2">
          <button type="button" onClick={() => navigate("/english/reading/articles")} title="返回原文精读" className="h-9 w-9 shrink-0 rounded-md flex items-center justify-center hover:bg-ink/5"><ArrowLeft size={18} /></button>
          ...
          {article.source_url && <a href={article.source_url} target="_blank" rel="noreferrer" title="打开原网页" className="h-9 w-9 shrink-0 rounded-md flex items-center justify-center hover:bg-ink/5"><ExternalLink size={15} /></a>}
          <button type="button" onClick={() => setFontSize((value) => Math.max(17, value - 1))} title="减小字号" className="h-9 w-9 shrink-0 rounded-md flex items-center justify-center hover:bg-ink/5"><Minus size={15} /></button>
          <button type="button" onClick={() => setFontSize((value) => Math.min(23, value + 1))} title="增大字号" className="h-9 w-9 shrink-0 rounded-md flex items-center justify-center hover:bg-ink/5"><Plus size={15} /></button>
        </div>
      </header>

      <article className="w-full max-w-xl min-w-0 mx-auto pt-8 [overflow-wrap:anywhere]">
        <p className="max-w-full whitespace-normal [overflow-wrap:anywhere] text-xs text-ink-lighter">{article.source_author || (article.source_url ? "网页原文" : "手动正文")}</p>
        <h1 className="max-w-full whitespace-normal [overflow-wrap:anywhere] text-xl font-semibold mt-2 mb-8 leading-8">{article.title}</h1>
        {!article.raw_content ? (
          <section className="border-y border-border py-5">
            <p className="text-sm font-medium">无法自动提取正文</p>
            <p className="text-xs text-ink-lighter mt-1">请粘贴文章内容后继续阅读。</p>
            <textarea value={fallbackContent} onChange={(event) => setFallbackContent(event.target.value)} rows={10} placeholder="粘贴完整英文正文" className="box-border w-full max-w-full min-w-0 mt-4 px-3 py-2.5 rounded-lg border border-border bg-card text-sm leading-6 outline-none resize-y" />
